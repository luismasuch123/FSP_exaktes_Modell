...
for path in instanceClassPaths:
    ...
    #TODO: anhand Instanznummer Datei öffnen? -> formale Beschreibung Instanzen notwendig
    for instance in instances:
        if ".yaml" in instance:
            ...
            if noFirstInstanceToBeSolved <= noInstance and noInstance <= N[noInstanceClass - 1]:
                ...
                #Visualisierung
                if displaySolution:
                    plot_solution(i_set, k_set, in_set, k_val, y_in, x_i_j_k, z_k_in, pos_i_dir, os.path.join(csvdir + "/" + Path(path).stem, instance.replace(".yaml", "")))
    noInstanceClass += 1